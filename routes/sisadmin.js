...
router.post("/editar_province", upload.any(), async(req, res)=>{
	...
	await funcao.provincia.forEach(element => {
		if (element._id == req.body.idioty){
			region = element.regiao;
			ex_supervisor = element.nome_supervisor;
		}
	});

	funcao.regiao.forEach(elment => {
		if (elment.nome == region){
			region_id = elment._id;
			regional = elment.regional_manager;
		}
	...
	await user_db.updateOne({nome: req.body.nome_supervisor}, {$set:{nome_supervisor: regional, funcao:"regional_manager", funcao_id: funcao.funcao[2]._id, regiao: region, regiao_id: region_id, provincia_trabalho: req.body.nome, provincia_id: req.body.idioty}}, function(err, data){
		if(err){
			console.log("ocorreu um erro ao tentar aceder os dados")
		}
		else{
...
router.post("/editar_region", upload.any(), async(req, res)=>{
	...
	var todo=await sisadmin_db.find();
	var gett = await sisadmin_db.findOne({_id:todo[0]._id}, {funcao: 1, regiao: 1}).lean();
	...
	await gett.funcao.forEach(element => {
		if (element.nome == "Director Regional"){
			funcaoid = element._id;
		}
	});
	...
	await user_db.updateOne({nome: ex_regional}, {$set:{funcao:"", funcao_id:""}}, function(err, data){
		if(err){
			...
			console.log("Unset on ex regional function done successfully")
		}
	})

	await user_db.updateOne({nome: req.body.regional_manager}, {$set:{nome_supervisor: "Luis Brazuna", funcao:"Regional Manager", funcao_id: funcaoid, regiao: req.body.nome, regiao_id: req.body.idioty, provincia_trabalho: "", provincia_id: ""}}, function(err, data){
		if(err){
			console.log("ocorreu um erro ao tentar aceder os dados")
		}
		else{
			console.log("Updated Regiao and funcao on usuario")
		}
	})
